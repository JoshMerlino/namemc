...
import { NameMCUser, PastName, PastSkin, SkinModel } from "namemc";
...
export default function scraper($: cheerio.Root): NameMCUser {
	...
	const pastSkins = Object.values(leftData.eq(1).children(".card-body")
		.children("a"))
	...
	const pastNames = Object.values(rightData.eq(1).children(".card-body.py-1")
		.children(".row"))
		.filter((elem) => elem.type === "tag")
		.map((elem) => $(elem))
		.map((elem) => {
			...
			const changedAtNum = new Date(elem
				.children(".col")
				.children(".row.no-gutters")
				.children(".col-12")
				.children("time")
			...
			const changedAt = isNaN(changedAtNum) ? null : changedAtNum;
