...
export default function scraper($: cheerio.Root): NameMCUser {
  ...
  const pastSkins = Object.values(
    leftData.eq(1).children(".card-body").children("a")
  )
    ...
    .map((skin) => {
      ...
      return new (class PastSkin {
        url = `https://texture.namemc.com/${hash?.substr(0, 2)}/${hash?.substr(
          2,
          2
        )}/${hash}.png`;
        ...
        changedAt = new Date(
          skin?.children("canvas")?.attr("title") + ""
        ).getTime();
      })() as PastSkin;
    })
  ...
  const pastNames = Object.values(
    rightData.eq(1).children(".card-body.py-1").children(".row")
  )
